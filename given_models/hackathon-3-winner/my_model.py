...
class Attention(nn.Module):
    ...
    def __init__(self, input_dims, attention_dims, n_heads=2):
        ...
        if n_heads == 2:
            ...
            self.attention_head_projection = nn.Linear(
                attention_dims * 2, input_dims)
        else:
            self.attention_head_projection = nn.Linear(
                attention_dims, input_dims)
    ...
    def forward(self, x):
        ...
        multihead_concat = self.attention_head_projection(
            multihead)     # shape: (B, 64, 64)
...
class Model(nn.Module):
    ...
    # nlayer - number of convlutional layers
    # embed_dim - size of the embeddiong
    # inner_dim - hidden layer size in residual blocks
    # attention_dim - size of attention mechanism(used to model long rang e dependencies)
    # use - boleean flag to enable 1 by 1 convolution
    # dropout - prevent overfittting
    ...
    def __init__(self, nlayers, embed_dim, inner_dim, attention_dim, use_1x1conv, dropout, device='cpu'):
        ...
        for i in range(nlayers):
            self.convLayers.append(
                Residual(self.embed_dim, self.inner_dim, self.use_1x1conv, self.dropout, 2**i))
    ...
    def forward(self, inputs):
        ...
        # we are chaning the dimension suitable to the cNN which expects from (batch_size, height, width,embed_dim (num_channels)) to(batch_size, num_channels, height, width)
